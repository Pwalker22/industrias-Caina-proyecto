...
$(document).ready(function() {
        ...
        try {
            ...
            users.forEach((user, index) => {
                ...
                <tr>
                    ...
                    <td>${user.lastName}</td>
                    <td>${user.email}</td>
                    <td>${user.phoneNumber || 'N/A'}</td> <!-- Actualiza el campo a phoneNumber -->
                    <td>${user.fkRole || 'N/A'}</td> <!-- Actualiza el campo a fkRole -->
                    <td>
                        <button class="btn btn-sm btn-primary" onclick="editUser(${user.Id}, '${user.userName}', '${user.lastName}', '${user.email}', '${user.phoneNumber}', ${user.fkRole})"><i class="fa-solid fa-pencil"></i></button>
 ...
 //?funcion actualizar usuario backend
    editUser = async (id, name, lastname, email, phoneNumber, fkRole) => {
        ...
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                ...
                <input id="phoneNumber" class="swal2-input" placeholder="Teléfono" value="${phoneNumber}">
                <input id="fkRole" class="swal2-input" placeholder="ID Rol" value="${fkRole}">
            `,
            focusConfirm: false,
            preConfirm: () => {
                return {
                    ...
                    fkRole: document.getElementById('fkRole').value,
                };
            }
        });
